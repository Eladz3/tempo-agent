...
function printHelp(): void {
  ...
  console.log("Usage:");
  console.log(
    "  tempo init                  Initialize .tempo/ in current directory",
  );
  console.log(
    "  tempo draft <name>          Create a new blank ideation file in .tempo/ideation/",
  );
  console.log(
    "  tempo compile [file]        Compile a markdown ideation file to a score (interactive if omitted)",
  );
  console.log(
    "  tempo tune                  Convert rough blurbs ([r] or [rough] prefixed) into clean design specs",
  );
  console.log(
    "  tempo run [score-name]      Run a score from .tempo/scores/<name>.json (interactive if omitted)",
  );
  console.log("");
}
...
`,
  ...
  console.log(
    chalk.blue("  .tempo/.env               — API keys (gitignored)"),
  );
  console.log(chalk.blue("  .tempo/config.json        — validation commands"));
  console.log(
    chalk.blue(
      "  .tempo/global-context.md  — project rules injected into every prompt",
    ),
  );
  console.log(
    chalk.blue("  .tempo/ideation/    — place your .md ideation files here"),
  );
...
function cmdDraft(args: string[]): void {
  ...
  if (!fs.existsSync(ideationDir)) {
    console.error(
      chalk.red("No .tempo/ideation/ directory found. Run `tempo init` first."),
    );
    process.exit(1);
  }
...
async function cmdCompile(args: string[]): Promise<void> {
  ...
  if (!fileName) {
    ...
    const chosen = await selectFromList(
      files,
      "Select an ideation file to compile:",
    );
    if (!chosen) {
      process.exit(0);
    }
